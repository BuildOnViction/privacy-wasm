...
func MRPProve(values []*big.Int, gammas []*big.Int) (MultiRangeProof, error) {
	...
	for j := range values {
		...
		// check(err)
		Comms[j] = pedersenCommitment(gammas[j], v)
		// gammas[j] = gamma
...
func JSMRPProve(_ js.Value, args []js.Value) interface{} {
	values := []*big.Int{}
	gammas := []*big.Int{}

	for i := 0; i < len(args)/2; i++ {
		bi, ok := new(big.Int).SetString(args[i].String(), 10)
		if !ok {
			return nil
		}
	...
	for i := len(args)/2; i < len(args); i++ {
		...
		gammas = append(gammas, bi)
	}

	result, err := MRPProve(values, gammas)
	if err != nil {
		return nil
	}
...
func main() {
	...
	println("REGISTERING WASM")

	js.Global().Set("mrpProve", js.FuncOf(JSMRPProve))
	<-c
}
